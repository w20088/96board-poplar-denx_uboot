...
LIBSPRODUCT ?= $(TOPDIR)/libproduct.a
LIBS += $(LIBSPRODUCT)
endif
...
.PHONY: mini-boot.bin
mini-boot.bin: $(TOPDIR)/fastboot.bin
	make -C $(TOPDIR)/arch/$(ARCH)/cpu/$(CPU)/compressed \
		CROSS_COMPILE=$(CROSS_COMPILE) \
		BINIMAGE=$(TOPDIR)/fastboot.bin TOPDIR=$(TOPDIR)
...
$(obj)u-boot:	ddr_training arm64_boot depend $(SUBDIRS) $(OBJS) $(LIBBOARD) $(LIBS) $(LDSCRIPT) $(obj)u-boot.lds
		$(GEN_UBOOT)
...
ifdef CONFIG_ARM64_SUPPORT
	make -C $(TOPDIR)/arch/$(ARCH)/cpu/$(CPU)/arm64_boot \
		TOPDIR=$(TOPDIR) \
		CROSS_COMPILE=$(CONFIG_ARM64_TOOLCHAINS)-
	touch common/arm64_boot.S
endif
...
$(LIBS):	depend $(SUBDIRS)
	if [ ss$@ != ss$(LIBSPRODUCT) ]; then ( \
		...
		cp -fv $(TOPDIR)/prebuilt/libproduct $(LIBSPRODUCT); \
	)fi
...
ifdef CONFIG_ARM64_SUPPORT
	make -C $(TOPDIR)/arch/$(ARCH)/cpu/$(CPU)/arm64_boot \
		TOPDIR=$(TOPDIR) \
		CROSS_COMPILE=$(CONFIG_ARM64_TOOLCHAINS)- clean
endif

clean: mini-boot.clean ddr_training.clean arm64_boot.clean
