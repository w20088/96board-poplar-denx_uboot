...
int do_bootm_linux(int flag, int argc, char *argv[], bootm_headers_t *images)
{
	...
	if (image_check_arch (&(images->legacy_hdr_os_copy), IH_ARCH_ARM64)) {
		extern char arm64_boot_entry[], arm64_boot_end[];
		volatile u32 *cpu_addr_reg = (u32 *)REG_PERI_CPU_RVBARADDR;
		volatile u32 *cpu_ctrl_reg = (u32 *)REG_PERI_CPU_AARCH_MODE;
		u32 val;
		/* arm64 boot image prepare */
		memcpy((void *)ARM64_WM_RST_VEC, (void *)arm64_boot_entry, arm64_boot_end - arm64_boot_entry);
#ifdef CONFIG_HI3798CV2X_FPGA
		*cpu_addr_reg = ARM64_WM_RST_VEC>>16;	/* config warm reset vector */
#else
		*cpu_addr_reg = ARM64_WM_RST_VEC;	/* config warm reset vector */
#endif
		*cpu_ctrl_reg = *cpu_ctrl_reg | 0xF;	/* enable 64bit instruction mode */
		...
		/* save fdt addr */
		(*(volatile unsigned long long *)0) = (unsigned long long)((unsigned long)(images->ft_addr));
		/* save kernel addr */
		(*(volatile unsigned long long *)8) = (unsigned long long)((unsigned long)theKernel);
