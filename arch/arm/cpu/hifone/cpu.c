...
static int hifone_boot_media(char **media)
{
	int boot_media;
...
static void get_hifone_clock(unsigned int *cpu, unsigned int *timer)
{
	unsigned long FREF = HIFONE_OSC_FREQ;
...
static int hifone_get_ca_type(void)
{
...
struct cpu_info_t cpu_info[] = {
	...
	},
	{
		.name   = "Hi3798Cv200_a",
		.chipid = _HIFONE_B02,
		.chipid_mask = _HIFONE_B02_MASK,
#ifdef CONFIG_NAND_FLASH_HISFC400
		.devs = DEV_HINFC610 | DEV_EMMCV200 | DEV_HISFC350 | DEV_HISFC400,
#else
		...
		.max_ddr_size = SZ_3G,
#endif
		.boot_media = hifone_boot_media,
		.get_clock  = get_hifone_clock,
		.get_cpu_version = NULL,
		.get_ca_type = hifone_get_ca_type,
	},
	{
		.name   = "Hi3798Cv200_b",
