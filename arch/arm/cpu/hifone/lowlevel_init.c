...
static void hifone_fpga_ddrtraining(void)
{
...
static void hifone_fpga_ddrtraining1(void)
{
#define DDRC0_BASE		0xF8A30000
...
#ifdef CONFIG_HIFONE_FPGA
		/* For A17, training */
		if ((get_cpu_midr()>>4)&0xFFF == 0xC0E) {
			hifone_fpga_ddrtraining();
			hifone_fpga_ddrtraining1();
		}
#else
#if defined(CONFIG_AVS_SUPPORT) && !defined(CONFIG_PMU_DEVICE)
		set_core_voltage();
