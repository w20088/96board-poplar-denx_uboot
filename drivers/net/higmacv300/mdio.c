...
static struct higmac_mdio_local higmac_mdio2;
#endif
...
#if (CONFIG_GMAC_NUMS > 2)
int higmac2_mdio_read(char *devname, unsigned char phy,
        unsigned char reg, unsigned short *value)
{
	return higmac_mdio_read(&higmac_mdio2, devname, phy, reg, value);
}
#endif
...
#if (CONFIG_GMAC_NUMS > 2)
int higmac2_mdio_write(char *devname, unsigned char phy,
		unsigned char reg, unsigned short val)
{
	return higmac_mdio_write(&higmac_mdio2, devname, phy, reg, val);
}
#endif
...
#if (CONFIG_GMAC_NUMS > 2)
	higmac_mdio2.iobase_phys = HIGMAC2_MDIO_IO_BASE;
	higmac_mdio2.iobase = HIGMAC2_MDIO_IO_BASE;
#endif

	higmac_mdio_init(&higmac_mdio0);
	higmac_mdio_init(&higmac_mdio1);
#if (CONFIG_GMAC_NUMS > 2)
	higmac_mdio_init(&higmac_mdio2);
#endif
}
