...
static void hieth_adjust_link(struct hieth_netdev_local *ld)
{
	...
	reg = 0;
	if (miiphy_read(UD_REG_NAME(PHY_NAME), UD_REG_NAME(PHY_ADDR), PHY_ANAR,
		    &reg) != 0) {
	...
	reg |= 0x1E0;
	miiphy_write(UD_REG_NAME(PHY_NAME), UD_REG_NAME(PHY_ADDR), PHY_ANAR,
		     reg);
retry:
	udelay(1);

	stat |=
	    miiphy_link(UD_REG_NAME(PHY_NAME),
			UD_REG_NAME(PHY_ADDR)) ? HIETH_LINKED : 0;
	stat |=
	    miiphy_duplex(UD_REG_NAME(PHY_NAME),
			  UD_REG_NAME(PHY_ADDR)) == FULL ? HIETH_DUP_FULL : 0;
	stat |=
	    miiphy_speed(UD_REG_NAME(PHY_NAME),
...
static int hieth_net_open(struct hieth_netdev_local *ld)
{
	...
	/* adjust amplitude */
	if (ld->port == UP_PORT) {		
		miiphy_write(UD_REG_NAME(PHY_NAME), UD_REG_NAME(PHY_ADDR), 0x1e,
			     0x1ff);
		miiphy_write(UD_REG_NAME(PHY_NAME), UD_REG_NAME(PHY_ADDR), 0x1d,
			     0x13);		
	}
#endif
...
static int hieth_init(void)
{
	...
	printf("Eth up port phy at 0x%02x is connect\n", U_PHY_ADDR);

	sprintf(D_PHY_NAME, "0:%d", D_PHY_ADDR);
	printf("Eth down port phy at 0x%02x is connect\n", D_PHY_ADDR);
	...
	if (!get_phy_device(D_PHY_NAME, D_PHY_ADDR)) {
		...
		if (ret) {
			hieth_error
			    ("register DownEther netdevice driver failed!");
			goto _error_register_driver;
		}
	}
...
int eth_init(bd_t * bd)
{
	...
	active = getenv("ethact");
	if (active != NULL) {
		if (strcmp(active, U_ETH_NAME) != 0
		    && strcmp(active, D_ETH_NAME) != 0) {
			printf("ethact(=%s) set error, "
			       "ethact SHOULD be set to %s or %s \n",
			       active, U_ETH_NAME, D_ETH_NAME);
			return -1;
		}
		printf("current ethact is %s.\n", active);
	}

retry:
	if (active == NULL || strcmp(active, U_ETH_NAME) == 0) {
		if (!get_phy_device(U_PHY_NAME, U_PHY_ADDR)) {
	...
	if (active == NULL || strcmp(active, D_ETH_NAME) == 0) {
		if (!get_phy_device(D_PHY_NAME, D_PHY_ADDR)) {
