...
static unsigned int himci_get_clock(void)
{
	...
	unsigned int crgclk[] = {
		100000000, EMMC_CLK_100M,
		50000000, EMMC_CLK_50M,
		25000000, EMMC_CLK_25M,
		200000000, EMMC_CLK_200M,
		150000000, EMMC_CLK_150M,
		300000000, EMMC_CLK_300M,
		337500000, EMMC_CLK_337DOT5M,
		0,0,
	};
...
static void himci_set_timing(unsigned int timing)
{
	...
	if (timing == MMC_TIMING_UHS_DDR50) {
		...
		tmp_reg |= EMMC_CLK_100M  | EMMC_DRV_PS_180_90;
	...
	if (timing == MMC_TIMING_MMC_HS200) {
		...
		tmp_reg |= EMMC_CLK_200M | EMMC_DRV_PS_135_67DOT5;
		himci_writel(tmp_reg,REG_BASE_CRG + REG_EMMC_PERI_CRG);
	}
	...
	if (timing == MMC_TIMING_MMC_HS400) {
		...
		tmp_reg = himci_readl(REG_BASE_CRG + REG_EMMC_PERI_CRG);
		tmp_reg &= ~(EMMC_CLK_MASK|EMMC_CLK_MODE|EMMC_DRV_PS_MASK | EMMC_SAP_PS_MASK);
		tmp_reg |= EMMC_CLK_150M | EMMC_DRV_PS_90_45 | (emmc_samplephase<< 12);
		himci_writel(tmp_reg,REG_BASE_CRG + REG_EMMC_PERI_CRG);
	}
}
